...
exports.generatePersonalizedModule = generatePersonalizedModule;
exports.generatePreQuiz = generatePreQuiz;
exports.generateAdaptiveUnit = generateAdaptiveUnit;
const generative_ai_1 = require("@google/generative-ai");
// Some changed
// ============================================
// SYSTEM PROMPT - This is the CRITICAL part
// ============================================
const generateModuleSystemPrompt = (moduleKey, knowledgeProfile, userSkillLevel) => `You are an elite AI Tutor for HeroCode: The Logical Dungeon.
 ...
 */
async function generatePersonalizedModule(moduleKey, knowledgeProfile, userSkillLevel) {
    try {
 ...
 */
async function generatePreQuiz(moduleKey, userSkillLevel) {
    try {
        const apiKey = process.env.GEMINI_API_KEY ?? '';
        const modelName = process.env.GEMINI_MODEL ?? 'gemini-2.5-flash';
        if (!apiKey) {
 ...
 */
async function generateAdaptiveUnit(moduleKey, unitNumber, previousUnitScore, knowledgeProfile, userSkillLevel) {
    try {
        const apiKey = process.env.GEMINI_API_KEY ?? '';
        const modelName = process.env.GEMINI_MODEL ?? 'gemini-2.5-flash';
        if (!apiKey) {
 ...
 */
function calculateAdaptiveDifficulty(previousScore, knowledgeProfile) {
    let baseDifficulty = 5;
    if (previousScore > 85)
        baseDifficulty += 2;
    else if (previousScore < 60)
        baseDifficulty -= 1;
    if (knowledgeProfile.adaptationData?.preferredPace === 'fast')
        baseDifficulty += 1;
    if (knowledgeProfile.adaptationData?.preferredPace === 'slow')
        baseDifficulty -= 1;
 ...
 */
function enrichModuleData(moduleData, moduleKey) {
    if (!moduleData.units) {
        moduleData.units = [];
    }
    moduleData.units = moduleData.units.map((unit, index) => ({
        ...
        quiz: {
            questions: (unit.quiz?.questions || []).slice(0, 5).map((q) => ({

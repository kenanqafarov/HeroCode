...
exports.submitBattleResult = exports.submitUnitQuiz = exports.submitPreQuiz = exports.startModule = exports.getLessonModules = void 0;
 ...
 */
const getLessonModules = async (req, res) => {
    try {
        const user = await User_1.default.findById(req.user.id);
        if (!user)
            return res.status(404).json({ success: false, message: 'User not found' });
        // Get modules from database (admin-created) and built-in modules
        const dbModules = await Module_1.default.find().select('_id title language difficulty description tags').lean();
        ...
        const moduleProgressMap = user.moduleProgress;
        const modulesList = allModules.map((mod) => {
        ...
        res.json({
            ...
            data: {
                ...
                moduleProgress: Object.fromEntries(moduleProgressMap?.entries() || [])
            }
        });
    }
    catch (error) {
...
exports.getLessonModules = getLessonModules;
/**
...
const startModule = async (req, res) => {
    try {
        ...
        const knowledgeProfileMap = user.knowledgeProfile;
        ...
        let preQuiz;
        try {
            preQuiz = await (0, gemini_service_1.generatePreQuiz)(moduleId, user.skillLevel || 'beginner');
        }
        catch (error) {
 ...
 */
const submitPreQuiz = async (req, res) => {
    try {
        const { moduleId } = req.params;
        ...
        if (Object.keys(answers).length > 0) {
            try {
                const preQuiz = await (0, gemini_service_1.generatePreQuiz)(moduleId, user.skillLevel || 'beginner');
                preQuiz.questions.forEach((q) => {
        ...
        if (!module) {
            try {
                module = await (0, gemini_service_1.generatePersonalizedModule)(moduleId, knowledgeProfile, user.skillLevel || 'beginner');
            }
            catch (error) {
...
const submitUnitQuiz = async (req, res) => {
    try {
        ...
        if (!isLevelEnd) {
            try {
                nextUnit = await (0, gemini_service_1.generateAdaptiveUnit)(moduleId, Number(unitId) + 1, score, knowledgeProfile || {}, user.skillLevel || 'beginner');
            }
            catch (error) {
...
exports.submitUnitQuiz = submitUnitQuiz;
/**
...
const submitBattleResult = async (req, res) => {
    try {
        ...
        const moduleProgress = moduleProgressMap.get(moduleId);
        if (!moduleProgress)
            return res.status(400).json({ success: false, message: 'Module not started' });
        if (!won) {
...
exports.submitBattleResult = submitBattleResult;
// ============ HELPER FUNCTIONS ============
...
function calculateStars(score, timeTaken) {
    if (score >= 90 && timeTaken < 600000)
        return 3;
    if (score >= 80)
        return 2;
    if (score >= 60)
 ...
 */
async function fetchModuleFromDatabase(moduleId) {
    try {
        // Try to find by MongoDB _id
        let module = await Module_1.default.findById(moduleId).lean();
        // If not found by _id, try to find by title or other identifier
        if (!module) {
            module = await Module_1.default.findOne({
                $or: [
 ...
 */
function convertModuleToLesson(moduleDoc) {
    const units = [];
    ...
    const mainUnit = {
        ...
        quiz: {
            questions: (moduleDoc.questions || []).map((q, idx) => ({
                id: `q${idx + 1}`,
                question: q.question,
...
function generateFallbackPreQuiz(moduleId) {
    return {
...
function generateFallbackModule(moduleId) {
    return {
        moduleId,
...
function generateFallbackUnit(unitId) {
    return {
        unitId,
        title: `Unit ${unitId}`,
        difficulty: 5,
